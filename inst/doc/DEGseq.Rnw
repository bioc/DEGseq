...
<<echo=TRUE,print=FALSE>>=
  ...
  write.table(exp[30:32,], row.names=FALSE)
@
\section{Make use of the functions provided by rtracklayer and ShortRead to import files containing aligned reads}
To import files containing aligned reads, users can make use of the functionas provided by rtracklayer and ShortRead.
For example,
<<echo=TRUE,print=FALSE>>=
...
@
To count the number of reads for each gene, we can use the method {\tt getGeneExp.aln}. For example,
<<echo=TRUE,print=FALSE>>=
  ...
  cat("output: ", output, "\n")
@
See also {\tt ?DEGseq.rng} and {\tt ?getGeneExp.rng} for more detail.

\name{getGeneExp.rng}
\alias{getGeneExp.rng}
\title{getGeneExp.rng: Count the number of reads and calculate the RPKM for each gene (deal with the objects of class RangedData)}
\description{
...
\usage{
getGeneExp.rng(rngBatch, strandInfo=FALSE,
               refFlat, output=paste(mapResultBatch[1],".exp",sep=""), min.overlapPercent=1)
}
\arguments{
  \item{rngBatch}{list containing the objects of class RangedData (package:IRanges).}
...
\examples{
  ...
  rngBatch <- list(kidneyR1L1_track)
  output <- file.path(tempdir(), "kidneyChr21Rng.bed.exp")
  exp <- getGeneExp.rng(rngBatch, refFlat=refFlat, output=output)
